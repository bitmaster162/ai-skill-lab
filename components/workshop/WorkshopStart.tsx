...
export function WorkshopStart({ locale = "ru" }: { locale?: WorkshopLocale }) {
  ...
  return (
    <WorkshopShell locale={locale} alternateHref={en ? "/start" : "/en/start"} contactHref="#contact-channels">
      <main id="main">
        <section className={styles.startHero}><span>{en ? "START · APPLICATION + CONTACT" : "START · ЗАЯВКА + КОНТАКТ"}</span><h1>{en ? <>Fit first.<br/><em>Program second.</em></> : <>Сначала fit.<br/><em>Потом программа.</em></>}</h1><p>{en ? `A session is ${sessionDurationMinutes} minutes. Use the application form or a direct channel; for a minor, coordination stays with an adult.` : `Сессия длится ${sessionDurationMinutes} минут. Оставьте заявку или используйте прямой канал; для несовершеннолетнего контакт ведёт взрослый.`}</p></section>
        <section className={styles.section} id="application-form"><div className={styles.sectionHead}><span>{en ? "APPLICATION" : "ЗАЯВКА"}</span><h2>{en ? "Send a short application. Direct channels remain available below." : "Оставьте короткую заявку. Прямые каналы остаются ниже."}</h2></div><LeadForm locale={locale}/><p className={styles.dataNote}>{en ? "Do not send identity documents, passwords, API keys, payment data or a child’s direct contact." : "Не присылайте документы, пароли, API keys, платёжные данные или прямой контакт ребёнка."}</p></section>
        <section className={styles.section}><div className={styles.sectionHead}><span>{en ? "DIRECT CONTACT" : "ПРЯМОЙ КОНТАКТ"}</span><h2>{en ? "Prefer a messenger or email? Four direct routes remain." : "Предпочитаете мессенджер или email? Четыре прямых канала остаются."}</h2></div><ChannelLinks locale={locale}/><p className={styles.dataNote}>{en ? "For minors, organizational communication uses an adult contact only." : "Для несовершеннолетних организационная коммуникация идёт только через контакт взрослого."}</p></section>
