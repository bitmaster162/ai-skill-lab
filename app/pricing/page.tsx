...
export default function PricingPage() {
  return <><Header contactHref="/start" alternateHref="/en/pricing"/><main id="main"><section className="hero heroR2"><div className="shell"><div className="eyebrow"><span className="dot"/> PRICING · 1:1</div><h1>Цена понятна<br/><span>до старта.</span></h1><p className="heroLead">На сайте нет checkout. Сначала фиксируем цель, формат и состав программы, затем подтверждаем оплату и правила переноса.</p><div className="heroActions"><Link className="button buttonPrimary" href="/start">Подобрать пакет →</Link><Link className="textLink" href="/method">Как устроено обучение →</Link></div></div></section>{groups.map(([name, items])=><section className="section" key={name as string}><div className="shell"><div className="sectionHead"><span className="kicker">{name as string}</span></div><div className="pricingGrid">{(items as string[][]).map(([plan,price,length,text],i)=><article className={`priceCard ${i===1?"featuredPrice":""}`} key={plan}>{i===1&&<span className="popular">Основной</span>}<span className="cardMeta">{plan}</span><h2>{length}</h2><div className="price">{price}</div><p>{text}</p></article>)}</div></div></section>)}<section className="section sectionMuted"><div className="shell premiumParentGrid"><div><span className="kicker">Family Concierge</span><h2>12 занятий + 2 сессии родителю</h2><p>Индивидуальный маршрут, семейные правила использования AI, итоговый проект и отдельный взрослый контур.</p></div><article className="premiumPriceCard"><div className="price">$1,490</div><Link className="button buttonPrimary buttonWide" href="/start">Обсудить Family</Link></article></div></section><section className="section"><div className="shell"><div className="sectionHead"><span className="kicker">До оплаты</span><h2>Коммерческие условия фиксируются письменно.</h2><p className="sectionSub">Цена на сайте — ориентир для указанного пакета. Количество занятий указано в пакете; точная длительность одной сессии согласуется и фиксируется до оплаты вместе с расписанием. До оплаты взрослому заказчику также подтверждаются точный scope, валюта и способ оплаты, правила переноса, отмены и возврата. Если индивидуальные письменные условия отличаются от сайта, приоритет имеют согласованные письменные условия.</p></div><div className="heroActions"><Link className="textLink" href="/terms">Условия обучения →</Link><Link className="textLink" href="/start">Отправить brief →</Link></div></div></section></main><Footer/></>;
}
