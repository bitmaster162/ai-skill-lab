...
export const metadata: Metadata = {
  title: { absolute: "Программы обучения AI — AI Skill Lab" },
  description: "Сравните программы AI Skill Lab для взрослых, детей 8–13 и подростков 14–18: темы, формат занятий, глубину, практику и итоговые артефакты.",
...
export default function CurriculumPage() {
  return <WorkshopInteractive locale="ru" alternateHref="/en/curriculum">
    <main id="main">
      ...
      <section className="section sectionInk"><div className="shell"><div className="sectionHead splitHead sectionHeadLight"><div><span className="kicker kickerLight">AI Skill Graph</span><h2>Разный контекст.<br/><em>Одна дисциплина работы.</em></h2></div><p>Переключайте Adult / Kids / Teens / Business. Меняются глубина и модель ответственности, но Think → Build → Verify → Ship остаётся явным.</p></div><SkillGraph locale="ru"/></div></section><section className="section sectionMuted"><div className="shell"><div className="sectionHead"><span className="kicker">Взрослым · пример ядра на 10 занятий</span><h2>От постановки задачи до рабочего AI-процесса.</h2><p className="sectionSub">Это ориентир, а не жёсткий календарь: если цель техническая или бизнесовая, глубина блоков меняется.</p></div><div className="curriculumList">{adult.map(([n,t,d])=><article key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p></article>)}</div></div></section>
