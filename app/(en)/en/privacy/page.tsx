...
export const metadata: Metadata = { title: "Privacy policy and data handling", description: "AI Skill Lab privacy policy: what the public request form processes, how direct contact channels work, and which personal data we do not collect.", alternates: { canonical: "/en/privacy", languages: { ru: "/privacy", en: "/en/privacy" } } };
...
export default function PrivacyPageEn() {
  return <LegalPage locale="en" path="privacy" title="Privacy" intro="How the site handles data in the current launch mode and what changes if the built-in application form is enabled.">
    ...
    <p>{site.leadFormEnabled ? <>The site sends an application to the HTTPS webhook configured by the operator. After confirmed storage, the service may send the operator a technical Telegram notification containing only the request ID, application time, selected audience and page locale. The applicant name, contact, learning goal, program and source path are not included in that notification. The primary application store must use appropriate access controls and retention practices.</> : <>In the current contact-only mode, the website does not transmit an application itself: opening Telegram hands the conversation to an external service operating under its own terms.</>}</p>
