...
export const metadata: Metadata = {
  ...
  description: "Выберите тип запроса и отправьте короткий brief в Telegram: взрослый, ребёнок, подросток, AI Studio или business workflow.",
  alternates: { canonical: "/start", languages: { ru: "/start", en: "/en/start" } },
  twitter: { card: "summary_large_image", title: "Начать обучение", description: "Выберите тип запроса и отправьте короткий brief в Telegram: взрослый, ребёнок, подросток, AI Studio или business workflow.", images: ["/opengraph-image"] },
};
...
export default function StartPage(){return <>
  ...
  <main id="main">
    ...
    <section className="section"><div className="shell"><div className="sectionHead"><span className="kicker">Выберите brief</span><h2>Пять сообщений, которые экономят лишний созвон.</h2><p className="sectionSub">Не надо отвечать идеально. Эти пункты просто дают достаточно контекста, чтобы предложить адекватный маршрут.</p></div><noscript><div className="briefNoScript"><strong>JavaScript отключён.</strong><p>Кнопки копирования недоступны, но brief можно переписать вручную и сразу открыть Telegram.</p><a className="button buttonGhost" href={site.telegram} target="_blank" rel="noopener noreferrer">Открыть Telegram →</a></div></noscript><div className="programGrid">{briefs.map((b)=><article className="programCard" key={b.meta} id={b.meta==="AI STUDIO" ? "studio-brief" : b.meta==="BUSINESS" ? "business-brief" : undefined}><span className="cardMeta">{b.meta}</span><h3>{b.title}</h3><ol className="numberList">{b.lines.map((x)=><li key={x}>{x}</li>)}</ol><div className="briefCardActions"><CopyBriefButton title={b.title} lines={b.lines} locale="ru"/><Link className="textLink" href={b.href}>Посмотреть направление →</Link></div></article>)}</div></div></section>
