...
export default function FaqPage() {
  return <><Header contactHref="/start" alternateHref="/en/faq"/><main id="main"><section className="hero heroR2"><div className="shell"><div className="eyebrow"><span className="dot"/> FAQ · BEFORE YOU START</div><h1>Сначала вопросы.<br/><span>Потом пакет.</span></h1><p className="heroLead">Короткие ответы на то, что имеет смысл понять до первого разговора.</p><div className="heroActions"><Link className="button buttonPrimary" href="/start">Обсудить задачу →</Link><Link className="textLink" href="/pricing">Стоимость →</Link></div></div></section><section className="section"><div className="shell"><div className="faqGrid"><div className="sectionHead stickyHead"><span className="kicker">FAQ</span><h2>10 коротких ответов</h2></div><div className="faqList">{items.map(([q,a])=><details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div></div></div></section></main><Footer/></>;
}
