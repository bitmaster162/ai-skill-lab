...
if release in {'R111B_BRAND_MARK_FAVICON','R111C_RU_TITLE_LOCALIZATION','R111D_SITEMAP_LASTMOD','R112_PLATFORM_SHORTCUT_TITLES','R114_BRAND_TOUCH_TARGET','R115_WORKSHOP_TYPOGRAPHY'}:
...
expected_files=61 if release=='R115_WORKSHOP_TYPOGRAPHY' else 59 if release in {'R110C_BRAND_LOGO','R111A_NOSCRIPT_FORM_FALLBACK','R111B_BRAND_MARK_FAVICON','R111C_RU_TITLE_LOCALIZATION','R111D_SITEMAP_LASTMOD','R112_PLATFORM_SHORTCUT_TITLES','R114_BRAND_TOUCH_TARGET','R115_WORKSHOP_TYPOGRAPHY'} else 58;require(manifest.get('file_count')==expected_files,f'{expected_files} release files')
...
listed={x['path']:(x['size'],x['sha256']) for x in manifest.get('files',[])};require(actual==listed,'manifest exact file bytes');payload=sum(x[0] for x in actual.values());payload_limit=608*1024 if release=='R115_WORKSHOP_TYPOGRAPHY' else 528384;require(payload<=payload_limit,f'payload {payload} > {payload_limit}')
