...
listed={x['path']:(x['size'],x['sha256']) for x in manifest.get('files',[])};require(actual==listed,'manifest exact file bytes');payload=sum(x[0] for x in actual.values());payload_limit=636*1024 if release in {'R143_R136_A1_A8_CLOSEOUT','R144_ICON_CACHE_PARITY','R145_PWA_INSTALLABILITY'} else 618*1024 if release=='R142_A9_A11_FOOTER_TOOLCHAIN_PROOF_TRUTH' else 608*1024 if release in {'R115_WORKSHOP_TYPOGRAPHY','R116_MONETARY_TYPOGRAPHY','R117_GLYPH_FALLBACK','R121_SITEMAP_LASTMOD_REFRESH','R130_STATIC_H2_OVERFLOW_GUARD'} else 528384;require(payload<=payload_limit,f'payload {payload} > {payload_limit}')
